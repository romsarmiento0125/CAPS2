        ...
        <v-col>
          <v-simple-table height="650px">
            <template v-slot:default>
              ...
              <tbody>
                <tr
                  v-for="(info, n) in userPickupCancel"
                  :key="n"
                >
...
<script>
  ...
  export default {
    ...
    computed: {
      userPickupCancel() {
        return this.$store.state.userPickupCancel;
      },
      usersToken(){
    ...
    methods: {
      getPickupCancel(){
        axios.get(this.getDomain()+'api/customerpickupcancel',
          {
            headers:{
          ...
          .then(res => {
            ...
            this.$store.commit('storeUserPickupCancel', res.data);
          })
